...
bool TaskNodeHandle::Update(bool parentRet, float elapsedTime)
{
	// タスクの終了状態
	...
	if (mpWorkTask != NULL)
	{
		...
		if (parentRet)
		{
			updateRet = mpWorkTask->Update(elapsedTime);
		}
	...
	for (std::list<TaskNodeHandle*>::iterator itr = mChildTaskList.begin(); itr != mChildTaskList.end(); ++itr)
	{
		// 更新処理
		bool ret = false;
		ret = (*itr)->Update(updateRet, elapsedTime);
...
//bool Task::TaskNodeHandle::Update(bool parentRet, DX::StepTimer timer)
//{
//	// タスクの終了状態
//	bool updateRet = parentRet;
//	bool funcRet = true;
...
//		if (parentRet)
//		{
//			updateRet = mpWorkTask->Update(timer);
//		}
//
//		// 終了状態の場合は管理タスクを削除
//		if (!updateRet)
...
//		// 更新処理
//		bool ret = false;
//		ret = (*itr)->Update(updateRet,timer);
//
//		if (funcRet && !ret)
//		{
//			funcRet = false;
